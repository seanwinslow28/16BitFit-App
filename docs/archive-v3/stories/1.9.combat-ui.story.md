...
> **Note:** "Champion" refers to the user's selected battle character (Sean, Mary, Marcus, Aria, Kenji, or Zara). All champions are purely cosmetic with identical stats (Mario Party model).
...
**Pass step count and champion from RN to Phaser:**
```typescript
// React Native
bridge.send('INIT_GAME', {
  selectedChampion: 'sean', // User's selected champion
  dailySteps: 7234,
...
handleInitGame(payload: any): void {
  this.loadChampion(payload.selectedChampion);
...
| Outcome | Result | Avatar State on Home |
|---------|--------|---------------------|
| `victory` | Player wins | Victory pose, stats gained |
| `continue_training` | Player didn't win | Determined pose, stats unchanged |

> ⚠️ **No-Defeat Philosophy:** XP is never removed, stats are never reduced. Battle outcome is either `victory` or `continue_training` (NOT "defeat" or "loss").

```typescript
// End of battle
const outcome = playerWon ? 'victory' : 'continue_training';
bridge.send('BATTLE_END', {
  outcome,
  xpGained: playerWon ? calculateXP() : 0,
...
- **Victory Pose:** When round ends with victory
- **Training Pose:** When round ends with continue_training
...
|------|---------|---------|
| 2025-11-18 | 1.0 | Initial implementation |
| 2026-01-02 | 2.0 | **Party Mode Alignment:** Updated terminology (combatCharacter→selectedChampion). Added Champion note (all 6 champions are cosmetic). Added Battle Outcomes section with No-Defeat Philosophy. Added `continue_training` outcome handling. |
